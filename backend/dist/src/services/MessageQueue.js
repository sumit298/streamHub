...
class MessageQueue {
    ...
    async publishChatMessage(data) {
        ...
        try {
            ...
            await this.channel.publish("chat.messages", "", Buffer.from(JSON.stringify(message)), {
    ...
    async publishAnalyticsEvent(event, data) {
        ...
        try {
            ...
            await this.channel.publish("analytics.events", event, Buffer.from(JSON.stringify(message)), {
                persistent: true,
    ...
    async publishUserPresence(action, data) {
        ...
        try {
            ...
            await this.channel.publish("user.presence", routingKey, Buffer.from(JSON.stringify(message)), {
