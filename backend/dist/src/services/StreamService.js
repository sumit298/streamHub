...
class StreamService {
    ...
    async createStream(userId, streamData) {
        try {
            ...
            try {
                const streamDoc = new index_1.Stream(stream);
    ...
    async joinStream(userId, streamId) {
        try {
            ...
            try {
                const streamDoc = await index_1.Stream.findOne({ id: streamId });
                if (streamDoc) {
    ...
    async produce(roomId, userId, transportId, rtpParameters, kind, isScreenShare = false) {
        try {
            const producer = await this.mediaService.produce(roomId, userId, transportId, rtpParameters, kind, isScreenShare);
            // Update database to mark stream as live
            try {
                const stream = (await index_1.Stream.findOneAndUpdate({ id: roomId }, {
                    isLive: true,
                    isPending: false,
                    startedAt: new Date(),
                }, { new: true }).populate("userId", "username avatar"));
                if (kind === "video" && !isScreenShare && stream) {
                    const followers = await index_1.Follow.find({
                        followingId: userId,
                    }).select("followerId");
                    ...
                    if (followerIds.length > 0) {
                        ...
                        const createdNotifications = await index_1.Notification.insertMany(notifications);
                        this.logger.info(`Created ${createdNotifications.length} notifications in database`);
                        if (this.io) {
                            followerIds.forEach((followerId) => {
                                this.io.to(`user:${followerId.toString()}`).emit("notification", {
                                    type: "stream-live",
                                    title: `${stream.userId.username} is live!`,
                                    message: stream.title,
                                    data: {
                        ...
                        this.logger.info(`Sent notifications to ${followerIds.length} followers for stream ${roomId}`);
                    }
                    else {
    ...
    async endStream(streamId, userId) {
        try {
            ...
            // update database
            try {
                await index_1.Stream.updateOne({ id: streamId }, {
                    ...streamUpdate,
                    endedAt: new Date(),
    ...
    async getStreamInfo(streamId) {
        try {
            const streamDoc = await index_1.Stream.findOne({ id: streamId }).populate("userId", "username _id email avatar");
            if (!streamDoc) {
                return null;
            }
    ...
    async getActiveStreams(options = {}) {
        try {
            ...
            const total = await index_1.Stream.countDocuments(query);
            // LEARNING: .lean() returns plain objects (faster than .toObject())
            const streams = await index_1.Stream.find(query)
    ...
    async getUserActiveStreams(userId) {
        try {
            return await index_1.Stream.find({ userId, isLive: true });
        }
        catch (error) {
    ...
    async updateStream(streamId, updateData) {
        try {
            ...
            await index_1.Stream.updateOne({ id: streamId }, updateData);
    ...
    async getUserStreams(userId, options = {}) {
        try {
            ...
            return await index_1.Stream.find(query)
                .limit(options.limit || 20)
                .sort({ createdAt: -1 });
        }
        catch (error) {
