...
const StreamController = {
    ...
    getStreams: (streamService, cacheService) => async (req, res) => {
        try {
            const { category, limit = "20", offset = "0", search, sortBy = "viewers", filter, status, } = req.query;
            const cacheKey = "streams:active";
            if (!search && offset === "0" && limit === "20") {
                const cached = await cacheService.get(cacheKey);
                if (cached) {
                    logger_1.default.info("Serving active streams from cache");
            ...
            logger_1.default.error("Get streams error", error);
    ...
    createStream: (streamService, cacheService) => async (req, res) => {
        try {
            const userActiveStreams = await streamService.getUserActiveStreams(req.userId);
            if (userActiveStreams.length > 0) {
                throw new error_types_1.ValidationError("You already have an active stream. Please end your current stream before starting a new one.");
            }
            const streamData = {
            ...
            logger_1.default.info(`Stream created: ${stream.id} by user ${req.userId}`);
            res.status(201).json({
                success: true,
            ...
            logger_1.default.error("Create stream error", error);
    ...
    getStreamById: (streamService, cacheService) => async (req, res) => {
        try {
            const streamId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
            const cacheKey = `stream:${streamId}`;
            const cached = await cacheService.get(cacheKey);
            if (cached) {
                logger_1.default.info(`Serving stream ${streamId} from cache`);
                res.json({ ...cached, cached: true });
                return;
            }
            ...
            logger_1.default.error("Get stream error", error);
            const normalizedError = (0, error_types_1.normalizeError)(error);
            res.status(normalizedError.statusCode).json({ success: false, error: normalizedError });
        }
    },
    // POST /api/streams/:id/end - End the stream
    endStream: (streamService, cacheService) => async (req, res) => {
        try {
            const streamId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
            const streamInfo = await streamService.getStreamInfo(streamId);
            if (!streamInfo) {
            ...
            const userIdStr = typeof streamInfo.userId === 'string' ? streamInfo.userId : streamInfo.userId._id.toString();
            if (userIdStr !== req.userId.toString() && req.user?.role !== "admin") {
                throw new error_types_1.ForbiddenError("Access denied. You can only end your own streams.");
            }
            ...
            logger_1.default.info(`Stream ended: ${streamId} by user ${req.userId}`);
            res.status(200).json({
                success: true,
            ...
            logger_1.default.error("End stream error", error);
            const normalizedError = (0, error_types_1.normalizeError)(error);
            res.status(normalizedError.statusCode).json({ success: false, error: normalizedError });
        }
    },
    // PATCH /api/streams/:id - Update stream details
    updateStream: (streamService, cacheService) => async (req, res) => {
        try {
            const streamId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
            const streamInfo = await streamService.getStreamInfo(streamId);
            if (!streamInfo) {
            ...
            const userIdStr = typeof streamInfo.userId === 'string' ? streamInfo.userId : streamInfo.userId._id.toString();
            if (userIdStr !== req.userId.toString() && req.user?.role !== "admin") {
                throw new error_types_1.ForbiddenError("Access denied. You can only update your own streams.");
            }
            const updateData = {};
            ...
            if (req.body.isLive !== undefined)
                updateData.isLive = req.body.isLive;
            ...
            logger_1.default.error("Update stream error", error);
            const normalizedError = (0, error_types_1.normalizeError)(error);
            res.status(normalizedError.statusCode).json({ success: false, error: normalizedError });
        }
    },
    // DELETE /api/streams/:id - Delete a stream
    deleteStream: (streamService, cacheService) => async (req, res) => {
        try {
            const streamId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
            const streamInfo = await streamService.getStreamInfo(streamId);
            if (!streamInfo) {
            ...
            const userIdStr = typeof streamInfo.userId === 'string' ? streamInfo.userId : streamInfo.userId._id.toString();
            if (userIdStr !== req.userId.toString() && req.user?.role !== "admin") {
                throw new error_types_1.ForbiddenError("Access denied. You can only delete your own streams.");
            }
            // StreamService doesn't have deleteStream, use endStream instead
            await streamService.endStream(streamId, req.userId);
            await cacheService.del(`stream:${streamId}`);
            await cacheService.del("streams:active");
            logger_1.default.info(`Stream deleted: ${streamId} by user ${req.userId}`);
            res.json({
                success: true,
            ...
            logger_1.default.error("Delete stream error", error);
            const normalizedError = (0, error_types_1.normalizeError)(error);
            res.status(normalizedError.statusCode).json({ success: false, error: normalizedError });
        }
    },
    // POST /api/streams/:id/join - Join a stream
    joinStream: (streamService) => async (req, res) => {
        try {
            const streamId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
            ...
            logger_1.default.error("Join stream error", error);
    ...
    getViewers: () => async (req, res) => {
        try {
            ...
            logger_1.default.error("Get viewers error", error);
            const normalizedError = (0, error_types_1.normalizeError)(error);
            res.status(normalizedError.statusCode).json({ success: false, error: normalizedError });
        }
    },
    // GET /api/streams/:id/stats - Get stream analytics
    getStreamStats: (streamService) => async (req, res) => {
        try {
            const streamId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
            const streamInfo = await streamService.getStreamInfo(streamId);
            if (!streamInfo) {
            ...
            const userIdStr = typeof streamInfo.userId === 'string' ? streamInfo.userId : streamInfo.userId._id.toString();
            if (userIdStr !== req.userId.toString() && req.user?.role !== "admin") {
                throw new error_types_1.ForbiddenError("Access denied. You can only view stats for your own streams.");
            }
            ...
            logger_1.default.error("Get stream stats error", error);
            const normalizedError = (0, error_types_1.normalizeError)(error);
            res.status(normalizedError.statusCode).json({ success: false, error: normalizedError });
        }
    },
    // GET /api/streams/user/:userId - Get streams by user
    getUserStreams: (streamService) => async (req, res) => {
        try {
            const userId = Array.isArray(req.params.userId) ? req.params.userId[0] : req.params.userId;
            const { includeEnded = "false", limit = "20" } = req.query;
            ...
            logger_1.default.error("Get user streams error", error);
            const normalizedError = (0, error_types_1.normalizeError)(error);
            res.status(normalizedError.statusCode).json({ success: false, error: normalizedError });
        }
    },
