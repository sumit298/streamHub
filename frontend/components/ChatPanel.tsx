  ...
  return (
    <div className="flex flex-col h-full bg-gray-900/50 backdrop-blur-sm rounded-2xl">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {messages.map((msg) => (
          <div key={msg.id} className="flex items-baseline gap-2 flex-wrap">
            <span className="text-xs text-gray-500 flex-shrink-0">{formatTime(msg.timestamp)}</span>
            <span className={`text-sm font-semibold flex-shrink-0 ${getUsernameColor(msg.username || 'Anonymous')}`}>
              {msg.username || 'Anonymous'}:
            </span>
            <span className="text-sm text-white break-words">{msg.content}</span>
          </div>
        ))}
